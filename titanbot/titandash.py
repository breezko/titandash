...
class TitandashTrayApp(object):
    ...
    def __init__(self):
        """
        Initializing a new system tray application. The tray object is created first, and then our server is restarted.
        """
        # Status boolean can be used to determine initialization status, allowing
        # us to exit the application startup process if a failure occurs.
        self.status = True

        # Setup all variables. Some are static and some are dynamic.
        self._variables()

        self.tray = self.create_tray(server_online=True)
    ...
    def _variables(self):
        """
        Initialize and set all variables used in menus and system tray.
    ...
    def start_instance_popup(self):
        ...
        if not info["windows"]:
            return self.message(self.START_INSTANCE, "No Windows Found.")

        # Create one shot window, grab data from user or cancel to start a new instance.
        window = sg.Window(
            title=self.START_INSTANCE,
            layout=[
                [sg.Text("Window:")],
                [sg.InputCombo(info["windows"], info["windows"][0])],
                [sg.Text("Instance:")],
                [sg.InputCombo(info["instances"], info["instances"][0])],
                [sg.Text("Configurations:")],
                [sg.InputCombo(info["configurations"], info["configurations"][0])],
                [sg.Submit(), sg.Cancel()]
            ]
        )
        ...
        # [1] - Instance
        # [2] - Configuration
        self.message(self.START_INSTANCE, "Starting {instance}\nWindow: {window}\nConfiguration: {configuration}".format(
            instance=values[1],
            window=values[0],
    ...
    def kill_instance_popup(self):
        """
        Generate an instance popup with options for killing a bot instance that is currently running or paused.

        This will prove less useful if the bot is currently running. But could be worth having if the server
        stops unexpectedly or ends up crashing but the instance is never killed naturally (ie: Through signal).
        """
        self.message(self.KILL_INSTANCE, "Loading Active Instances")
        active = json.loads(subprocess.check_output(["python", "manage.py", "instance_active"]))

        if not active["instances"]:
            return self.message(self.KILL_INSTANCE, "No Active Instance(s) Found.")

        window = sg.Window(
            title=self.KILL_INSTANCE,
            layout=[
                [sg.Text("Instance:")],
                [sg.InputCombo(active["instances"], active["instances"][0])],
                [sg.Submit(), sg.Cancel()]
            ]
        )
    ...
    def start(self):
        ...
        while True:
            ...
            # START INSTANCE: Initialize a bot from the tray.
            if event == self.START_INSTANCE:
                self.start_instance_popup()

            # KILL INSTANCE: Attempt to kill a running bot instance.
            if event == self.KILL_INSTANCE:
                self.kill_instance_popup()
